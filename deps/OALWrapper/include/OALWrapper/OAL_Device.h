...
public:
  ...
  inline int          GetNumSources() {
    if (mpSourceManager) return mpSourceManager->GetNumVoices();
    else
      return 0;
  }
  inline bool* GetExtensions() { return mvbExtensions; }
  inline bool  IsExtensionAvailable(int alWhich) {
    if ((alWhich >= 0) && (alWhich < NUM_EXTENSIONS)) return mvbExtensions[alWhich];
    else
      return false;
  }
