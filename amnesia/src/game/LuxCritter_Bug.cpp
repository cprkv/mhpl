...
void cLuxCritter_Bug::UpdateVelocity(float afTimeStep) {
  ...
  if (mfPlaySoundCount < 0) {
    mfPlaySoundCount = bScared ? cMath::RandRectf(mvScaredSoundRandMinMax.x, mvScaredSoundRandMinMax.y) : cMath::RandRectf(mvNormalSoundRandMinMax.x, mvNormalSoundRandMinMax.y);
