...
bool cLuxEnemy_WaterLurker::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage* apMessage) {
  ...
  ////////////////////////////////
  //Move forward
  kLuxOnUpdate if (Dist2D(mvTempPos) > mpCharBody->GetSize().x * 0.75f)
      mpMover->MoveToPos(mvTempPos);

  ////////////////////////////////
  //Player detection, only hunt if player is really close.
  kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) if (DistToPlayer2D() < 2.0f)
      ChangeState(eLuxEnemyState_Hunt);
  ...
  ///////////////////////
  // Movement effect
  kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) if (mpMover->GetWantedSpeedAmount() > 0.2f)
  ...
  ///////////////////////
  //On update
  kLuxOnUpdate if (DistToPlayer() < mfNormalAttackDistance) {
