...
void cLuxPlayerState_InteractPush::Update(float afTimeStep) {
  ...
  float fDistance = mpPushData->mbPushAtPoint ? cMath::Vector3Dist(pCharBody->GetFeetPosition(), vAttachPos) : cMath::Vector3Dist(pCharBody->GetFeetPosition(), mpCurrentBody->GetLocalPosition());
