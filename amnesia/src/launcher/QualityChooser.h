...
public:
  cVideoCardManufacturer(const tString& asName)
      : msName(asName) {}
...
public:
  cVideoCardModel(const tString& asName)
      : msName(asName) {}
  const tString& GetName() { return msName; }
...
public:
  cVideoCardSeries(cVideoCardModel* apModel, const tString& asCode, eQRating aRating)
      : mpModel(apModel)
      , msCode(asCode)
      , mRating(aRating) {}
