    ...
    for (NSString* k in _settingsMap) {
      ...
      if (preset && [preset boolValue] == YES) {
        ...
        if ([t isEqualToString:@"Int"]) {
          ...
          bEqualsPreset = bEqualsPreset && (_mainConfig->GetInt(sLevel, sName, def) == pPreset->GetInt(sLevel, sName, def));
          ...
          bEqualsPreset = bEqualsPreset && (_mainConfig->GetBool(sLevel, sName, def) == pPreset->GetBool(sLevel, sName, def));
          ...
          bEqualsPreset = bEqualsPreset && (_mainConfig->GetFloat(sLevel, sName, def) == pPreset->GetFloat(sLevel, sName, def));
...
- (IBAction)enterCustomResolution:(id)sender {
  ...
  [[NSApplication sharedApplication] beginSheet:customResolutionWindow
...
- (void)sheetDidEnd:(NSWindow*)sheet returnCode:(NSInteger)returnCode contextInfo:(void*)contextInfo {
  ...
  if (returnCode == NSOKButton) {
    ...
    for (NSDictionary* d in [_resolutions arrangedObjects]) {
      if ([[d valueForKey:@"Width"] isEqualToNumber:width] && [[d valueForKey:@"Height"] isEqualToNumber:height]) {
