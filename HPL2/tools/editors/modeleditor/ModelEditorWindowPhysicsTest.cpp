...
void cPhysicsTestRenderCallback::OnPostSolidDraw(cRendererCallbackFunctions* apFunctions) {
  ...
  if (mpBodyPicker->mpPickedBody == NULL)
    return;
...
cModelEditorWindowPhysicsTest::cModelEditorWindowPhysicsTest(iEditorBase* apEditor, iFrameBuffer* apFB)
    : iEditorWindow(apEditor,
                    "Physics Test Window")
...
void cModelEditorWindowPhysicsTest::SetUpModel() {
  ...
  if (pModel->GetEntities().size() == 0 || pModel->GetMesh() == NULL)
    return;
...
bool cModelEditorWindowPhysicsTest::OnViewportMouseUp(const cGuiMessageData& aData) {
  ...
  if (aData.mlVal & eGuiMouseButton_Left) {
    if (mBodyPicker.mpPickedBody)
      mBodyPicker.mpPickedBody->SetAutoDisable(true);
    mBodyPicker.Clear();
  }
