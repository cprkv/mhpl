...
namespace hpl {
  ...
  void iLight::OnChangeVisible() {
    for (auto& billboard : billboards_) {
      billboard.mpBillboard->SetVisible(mbIsVisible);
    }
  }
  ...
  bool iLight::ShadowCasterIsValid(iRenderable* apObject) {
    auto it = m_mapShadowCasterCache.find(apObject);
  ...
  bool iLight::ShadowCastersAreUnchanged(const tRenderableVec& avObjects) {
    size_t dynObjectCount = 0;

    for (auto* object : avObjects) {
      if (!object->IsStatic()) {
        dynObjectCount++;
        if (!ShadowCasterIsValid(object)) {
          return false;
        }
      }
    }

    return dynObjectCount == m_mapShadowCasterCache.size();
  }

  void iLight::SetShadowCasterCacheFromVec(const tRenderableVec& avObjects) {
    m_mapShadowCasterCache.clear();

    for (auto* avObject : avObjects) {
      if (!avObject->IsStatic()) {
        AddShadowCaster(avObject);
      }
    }
  }
  ...
  void iLight::LoadXMLProperties(const tString asFile) {
    ...
    if (sPath != _W("")) {
      ...
      auto* pDoc = hplNew(TiXmlDocument, ());
  ...
  void iLight::AttachBillboard(cBillboard* apBillboard, const cColor& aBaseColor) {
    ...
    billboards_.push_back(bbConnection);
  }
  ...
  void iLight::OnFlickerOn() {
    //Particle system
    if (!msFlickerOnPS.empty()) {
      cParticleSystem* ps = mpWorld->CreateParticleSystem(GetName() + "_PS", msFlickerOnPS, cVector3f(1, 1, 1));
      if (ps) {
        ps->SetMatrix(GetWorldMatrix());
      }
    }
  }
  ...
  void iLight::OnSetDiffuse() {
    for (auto& billboard : billboards_) {
      billboard.mpBillboard->SetColor(billboard.mBaseColor * cColor(mDiffuseColor.r, mDiffuseColor.g, mDiffuseColor.b, 1));
    }
  }
