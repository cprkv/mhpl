...
#define kCRCKey (0x16AF2C1D)
  ...
  bool cConfigFile::Load() {
    ...
    if (mbUseCRC) {
      ...
      /////////////////////////////////
      // CRC
      if (!buff.CheckInternalCRC(kCRCKey)) {
  ...
  void cConfigFile::SetString(const tString& asLevel, const tString& asName, const tString& asVal) {
    ...
    if (pLevelElem == nullptr) {
      auto* pNodeChild = hplNew(TiXmlElement, (asLevel.c_str()));
      pLevelElem       = dynamic_cast<TiXmlElement*>(mpXmlDoc->InsertEndChild(*pNodeChild));
