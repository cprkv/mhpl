...
namespace hpl {
  ...
  static iCollideShape* GetBodyShape(cXmlElement*            apBodyElem,
                                     iPhysicsWorld*          apPhysicsWorld,
                                     tLoaderCollideShapeMap& a_setShapes) {
    ////////////////////////////////////////
    // Get shapes for body
  ...
  static iCollideShape* CreateCollideShape(cXmlElement*     apShapeElem,
                                           iPhysicsWorld*   apPhysicsWorld,
                                           const cVector3f& avScale) {
    eCollideShapeType type =
        ToCollideShape(apShapeElem->GetAttributeString("ShapeType"));
    cVector3f vSize = apShapeElem->GetAttributeVector3f("Scale") * avScale;
    cVector3f vPos =
        apShapeElem->GetAttributeVector3f("RelativeTranslation") * avScale;
  ...
  static iPhysicsJoint*
  CreateJoint(const tString& asEntityName, cXmlElement* apJointElem,
              iPhysicsWorld* apPhysicsWorld, tLoaderPhysicsBodyMap& a_setBodies,
    ...
    // Name and type
    tString sJointName =
        asEntityName + "_" + apJointElem->GetAttributeString("Name");
    ...
    iPhysicsBody* pParentBody =
        lParentID > 0 ? FindBody(lParentID, a_setBodies) : NULL;
    ...
    if (jointType == ePhysicsJointType_Hinge) {
      iPhysicsJointHinge* pJoint = apPhysicsWorld->CreateJointHinge(
          sJointName, vPivot, vPinDir, pParentBody, pChildBody);

      pJoint->SetMinAngle(
          cMath::ToRad(apJointElem->GetAttributeFloat("MinAngle")));
      pJoint->SetMaxAngle(
          cMath::ToRad(apJointElem->GetAttributeFloat("MaxAngle")));
    ...
    // Ball
      ...
      iPhysicsJointBall* pJoint = apPhysicsWorld->CreateJointBall(
          sJointName, vPivot, vPinDir, pParentBody, pChildBody);

      pJoint->SetConeLimits(
          cMath::ToRad(apJointElem->GetAttributeFloat("MaxConeAngle")),
    ...
    // Slider
      ...
      iPhysicsJointSlider* pJoint = apPhysicsWorld->CreateJointSlider(
          sJointName, vPivot, vPinDir, pParentBody, pChildBody);
    ...
    // Screw
      ...
      iPhysicsJointScrew* pJoint = apPhysicsWorld->CreateJointScrew(
          sJointName, vPivot, vPinDir, pParentBody, pChildBody);
  ...
  static cMatrixf GetMatrixFromVectors(const cVector3f& avPos,
                                       const cVector3f& avRot,
                                       const cVector3f& avScale) {
    cMatrixf mtxOut = cMath::MatrixScale(avScale);
    mtxOut          = cMath::MatrixMul(cMath::MatrixRotate(avRot, eEulerRotationOrder_XYZ),
                              mtxOut);
  ...
  iEntity3D* cEntityLoader_Object::Load(const tString& asName, int alID,
                                        bool abActive, cXmlElement* apRootElem,
                                        const cMatrixf&  a_mtxTransform,
                                        const cVector3f& avScale, cWorld* apWorld,
    ...
    cXmlElement* pMeshElem = NULL;
    {
      ...
      if (cString::GetFilePath(sMeshFile).size() < 1) {
        sMeshFile = cString::SetFilePath(
            sMeshFile, cString::To8Char(cString::GetFilePathW(asFullPath)));
      }
      // Log("Mesh: '%s'\n",sMeshFile.c_str());
    ...
    // Load sub meshes
    {
      ...
      while (submeshIt.HasNext()) {
        ...
        // tString sMaterialFile =
        // cString::ToString(pSubMeshElem->Attribute("MaterialFile"),"");
        ...
        if (bHasSkeleton == false) {
          cMatrixf mtxLocalTransform = GetMatrixFromVectors(
              pSubMeshElem->GetAttributeVector3f("WorldPos") * mvScale,
              pSubMeshElem->GetAttributeVector3f("Rotation"),
              pSubMeshElem->GetAttributeVector3f("Scale") * mvScale);

          // mtxLocalTransform = cMath::MatrixMul(mtxLocalTransform,
          // cMath::MatrixScale(mvScale));
        ...
        if (lID < 0)
          lID = pSubMeshElem->GetAttributeInt(
              "SubMeshID"); // To support older files!
      ...
      {
      cMaterial *pMaterial =
      apWorld->GetResources()->GetMaterialManager()->CreateMaterial(sMaterialFile);
      if(pMaterial)
      {
    ...
    if (mbLoadAnimations && mpEntity && pAnimationsElem) {
      ...
      while (animElemIt.HasNext()) {
        ...
        float   fSpecialEventTime =
            pAnimElem->GetAttributeFloat("SpecialEventTime", 0.0f);

        if (cString::GetFilePath(sFile).length() <= 1)
          sFile = cString::SetFilePath(
              sFile, cString::To8Char(cString::GetFilePathW(asFullPath)));

        cAnimation* pAnim =
            apWorld->GetResources()->GetAnimationManager()->CreateAnimation(
                sFile);
        ...
        if (pAnim) {
          ...
          while (eventElemIt.HasNext()) {
            ...
            pEvent->mType =
                ToAnimEventType(pEventElem->GetAttributeString("Type"));
    ...
    // Load World entities
    {
        ...
        while (entityIt.HasNext()) {
          ...
          if (sEntityType == "ParticleSystem") {
            if (mbLoadParticleSystems) {
              cParticleSystem* pPS = cEngineFileLoading::LoadParticleSystem(
                  pEntityElem, asName + "_", apWorld);
              if (pPS)
          ...
          // Billboard
            ...
            if (mbLoadBillboards) {
              cBillboard* pBillboard = cEngineFileLoading::LoadBillboard(
                  pEntityElem, asName + "_", apWorld, apWorld->GetResources(),
                  mbLoadAsStatic, &lstLightBillboardListConnections);
              if (pBillboard)
          ...
          // Sound
            ...
            if (mbLoadSounds) {
              cSoundEntity* pSound = cEngineFileLoading::LoadSound(
                  pEntityElem, asName + "_", apWorld);
              if (pSound)
          ...
          // Light
            ...
            if (mbLoadLights) {
              iLight* pLight = cEngineFileLoading::LoadLight(
                  pEntityElem, asName + "_", apWorld, apWorld->GetResources(),
                  mbLoadAsStatic);
              if (pLight)
      ...
      tEFL_LightBillboardConnectionListIt connIt =
          lstLightBillboardListConnections.begin();
      for (; connIt != lstLightBillboardListConnections.end(); ++connIt) {
        ...
        if (pLight == NULL) {
          Warning("Light with name '%s' does not exist!",
                  lightConnect.msLightName.c_str());
          continue;
        }
        if (pBB == NULL) {
          Warning("Billboard with id '%d' does not exist!",
                  lightConnect.msBillboardID);
          continue;
        }
        ...
        while (boneIt.HasNext()) {
          ...
          if (pBoneState == NULL) {
            Error("Could not find bone '%s' in model '%s'\n", sName.c_str(),
                  cString::To8Char(asFullPath).c_str());
            continue;
          }
          ...
          LoadAndAttachChildren(pBoneElem, NULL, pBoneState, lstTempEntities,
                                mapBoneStates, true, false);
        }
      }
    }
    ...
    if (pPhysicsWorld) {
      ...
      if (pShapesElem) {
        ...
        while (shapeIt.HasNext()) {
          ...
          iCollideShape* pCollideShape =
              CreateCollideShape(pBodyShapeElem, pPhysicsWorld, mvScale);
          if (pCollideShape == NULL)
            continue;
          int lID = pBodyShapeElem->GetAttributeInt("ID");

          setShapes.insert(
              tLoaderCollideShapeMap::value_type(lID, pCollideShape));
        }
      }
    }
      ...
      if (pBodiesElem) {
        ...
        while (bodyIt.HasNext()) {
          ...
          iCollideShape* pShape =
              GetBodyShape(pBodyElem, pPhysicsWorld, setShapes);
          if (pShape == NULL) {
          ...
          iPhysicsBody* pBody =
              pPhysicsWorld->CreateBody(asName + "_" + sBodyName, pShape);
          SetBodyProperties(pBody, pBodyElem);

          // Material
          iPhysicsMaterial* pPhysicsMat = pPhysicsWorld->GetMaterialFromName(
              pBodyElem->GetAttributeString("Material"));
          if (pPhysicsMat)
          ...
          /////////////////////
          // Attach children
          LoadAndAttachChildren(pBodyElem, pBody, nullptr, lstTempEntities,
                                mapBoneStates, true, true);
        }
      }

      ////////////////////////
      // If no bones got attached to bodies, then add the entire entity to be
      // attached. (Luis: this comment does not make much sense given the code
      // below, not sure what the point is)
      if (mpMesh->GetSkeleton()) {
        if (mapBoneStates.size() != mpEntity->GetBoneStateNum()) {
          Error("Loading entity %s: Skeletons in mesh file (%ls) and .ent file "
                "(%ls) differ! Probably caused by .ent not being up to date with "
                "mesh\n",
                asName.c_str(), mpMesh->GetFullPath().c_str(),
    ...
    if (!mvBodies.empty()) {
      ...
      for (iEntity3D* pEntity : lstTempEntities) {
        if (pEntity->GetEntityType() == "SubMesh" &&
            mpMesh->GetSkeleton() == nullptr &&
    ...
    if (mvBodies.size() <= 0) {
      if (mpMesh->GetSkeleton())
        mpEntity->SetMatrix(
            cMath::MatrixMul(a_mtxTransform, cMath::MatrixScale(avScale)));
      else
    ...
    if (pPhysicsWorld) {
      ...
      if (JointsElem) {
        ...
        while (jointIt.HasNext()) {
          ...
          iPhysicsJoint* pJoint = CreateJoint(asName, pJointElem, pPhysicsWorld,
                                              setBodies, a_mtxTransform, mvScale);
          if (pJoint) {
    ...
    for (tEntity3DListIt it = lstEntities.begin(); it != lstEntities.end();
         ++it) {
  ...
  void cEntityLoader_Object::AttachEntityChild(iEntity3D*      apParent,
                                               const cMatrixf& a_mtxInvParent,
                                               iEntity3D*      apChild) {
    // Calculate local matrix
    cMatrixf mtxLocalTransform =
        cMath::MatrixMul(a_mtxInvParent, apChild->GetWorldMatrix());
    apChild->SetMatrix(mtxLocalTransform);

    // Add the entity to parent and remove it from any other hierarchy it might be
    // in.
  ...
  void cEntityLoader_Object::AttachBoneChild(cBoneState*     apBoneState,
                                             const cMatrixf& a_mtxInvParent,
                                             iEntity3D*      apChild) {
    // Calculate local matrix
    cMatrixf mtxLocalTransform =
  ...
  void cEntityLoader_Object::AttachBoneToBody(iPhysicsBody*   apParentBody,
                                              const cMatrixf& a_mtxInvParent,
                                              cBoneState*     apBoneState) {
    cMatrixf mtxInvBone = cMath::MatrixInverse(apBoneState->GetWorldMatrix());
    cMatrixf mtxBoneToBody =
        cMath::MatrixMul(mtxInvBone, apParentBody->GetWorldMatrix());

    // Attaching work differently for bones since they rely on base matrix and
    // animations. This sets up the basic properties and then the skeletal physics
    // needs to be turned on the mesh entity.
    ...
    /*
  iPhysicsBody *pColliderBody =
  mpPhysicsWorld->CreateBody(apEntity->GetName()+"_collider_"+pBoneState->GetName(),pShape);
  ...
  */
  ...
  void cEntityLoader_Object::LoadAndAttachChildren(
      cXmlElement* apMainElem, iEntity3D* apEntityParent,
      cBoneState* apBoneStateParent, tEntity3DList& a_lstChildList,
      tNodeStateMap& a_mapBoneStates, bool abRemoveAttachedChild, bool abIsBody) {
    ...
    while (childIt.HasNext()) {
      ...
      bool bFound = false;
      for (tEntity3DListIt it = a_lstChildList.begin();
           it != a_lstChildList.end(); ++it) {
        //////////////////////////
      ...
      if (bFound == false) {
        if (apEntityParent)
          Warning("Could not find child with unique ID %d to attach to entity "
                  "'%s' in '%s'\n",
                  lID, apEntityParent->GetName().c_str(), msFileName.c_str());
        else
          Warning("Could not find child with unique ID %d to attach to bone '%s' "
                  "in '%s'\n",
  ...
  void cEntityLoader_Object::SetBodyProperties(iPhysicsBody* apBody,
                                               cXmlElement*  apElem) {
    apBody->SetMatrix(
        GetMatrixFromVectors(apElem->GetAttributeVector3f("WorldPos") * mvScale,
                             apElem->GetAttributeVector3f("Rotation"), 1.0f));
    ...
    apBody->SetCollideCharacter(
        apElem->GetAttributeBool("CollideCharacter", true));
    ...
    apBody->SetBuoyancyDensityMul(
        apElem->GetAttributeFloat("BuoyancyDensityMul", 1.0));
    ...
    apBody->SetContinuousCollision(
        apElem->GetAttributeBool("ContinuousCollision", true));

    apBody->SetPushedByCharacterGravity(
        apElem->GetAttributeBool("PushedByCharacterGravity", false));

    apBody->SetVolatile(apElem->GetAttributeBool("Volatile", false));

    apBody->SetUseSurfaceEffects(
        apElem->GetAttributeBool("UseSurfaceEffects", true));

    apBody->SetGravityCanAttachCharacter(
        apElem->GetAttributeBool("CanAttachCharacter", false));
  ...
  void cEntityLoader_Object::SetJointProperties(iPhysicsJoint* pJoint,
    ...
    pJoint->SetMinMoveVolume(
        apJointElem->GetAttributeFloat("MinMoveVolume", 0.3f));
    pJoint->SetMinMoveFreqSpeed(
        apJointElem->GetAttributeFloat("MinMoveFreqSpeed", 0.9f));
    pJoint->SetMaxMoveFreq(apJointElem->GetAttributeFloat("MaxMoveFreq", 1.1f));
    pJoint->SetMaxMoveVolume(
        apJointElem->GetAttributeFloat("MaxMoveVolume", 1.0f));
    pJoint->SetMaxMoveFreqSpeed(
        apJointElem->GetAttributeFloat("MaxMoveFreqSpeed", 1.1f));
    pJoint->SetMiddleMoveSpeed(
        apJointElem->GetAttributeFloat("MiddleMoveSpeed", 1.0f));
    pJoint->SetMiddleMoveVolume(
        apJointElem->GetAttributeFloat("MiddleMoveVolume", 1.0f));
    pJoint->SetMoveSpeedType(cString::ToLowerCase(apJointElem->GetAttributeString(
                                 "MoveType", "Linear")) == "angular"
                                 ? ePhysicsJointSpeed_Angular
                                 : ePhysicsJointSpeed_Linear);

    pJoint->SetStickyMinLimit(
        apJointElem->GetAttributeBool("StickyMinLimit", false));
    pJoint->SetStickyMaxLimit(
        apJointElem->GetAttributeBool("StickyMaxLimit", false));
    ...
    pJoint->SetLimitAutoSleep(
        apJointElem->GetAttributeBool("LimitAutoSleep", false));
    pJoint->SetLimitAutoSleepDist(
        apJointElem->GetAttributeFloat("LimitAutoSleepDist", 0.02f));
    pJoint->SetLimitAutoSleepNumSteps(
        apJointElem->GetAttributeInt("LimitAutoSleepNumSteps", 10));

    pJoint->SetCollideBodies(
        apJointElem->GetAttributeBool("CollideBodies", true));

    pJoint->GetMaxLimit()->msSound =
        apJointElem->GetAttributeString("MaxLimitSound", "");
    pJoint->GetMaxLimit()->mfMaxSpeed =
        apJointElem->GetAttributeFloat("MaxLimitMaxSpeed", 10.0f);
    pJoint->GetMaxLimit()->mfMinSpeed =
        apJointElem->GetAttributeFloat("MaxLimit_MinSpeed", 20.0f);
    if (pJoint->GetMaxLimit()->mfMaxSpeed <= 0)
      pJoint->GetMaxLimit()->mfMaxSpeed = 0.01f;

    pJoint->GetMinLimit()->msSound =
        apJointElem->GetAttributeString("MinLimitSound", "");
    pJoint->GetMinLimit()->mfMaxSpeed =
        apJointElem->GetAttributeFloat("MinLimitMaxSpeed", 10.0f);
    pJoint->GetMinLimit()->mfMinSpeed =
        apJointElem->GetAttributeFloat("MinLimitMinSpeed", 20.0f);
  ...
  for(; pControllerElem != NULL; pControllerElem =
  pControllerElem->NextSiblingElement("Controller"))
  {
  ...
  void cEntityLoader_Object::LoadController(iPhysicsJoint* apJoint,
                                            iPhysicsWorld* apPhysicsWorld,
                                            TiXmlElement*  apElem) {
    //////////////////////////////
    // Get the properties
  ...
  ePhysicsControllerType CtrlType =
  GetControllerType(apElem->Attribute("Type")); float fA=
  cString::ToFloat(apElem->Attribute("A"),0); float fB=
  cString::ToFloat(apElem->Attribute("B"),0); float fC=
  cString::ToFloat(apElem->Attribute("C"),0); int lIntegralSize =
  cString::ToInt(apElem->Attribute("IntegralSize"),1);

  ePhysicsControllerInput CtrlInput =
  GetControllerInput(apElem->Attribute("Input")); ePhysicsControllerAxis
  CtrlInputAxis = GetControllerAxis(apElem->Attribute("InputAxis")); float
  fDestValue = cString::ToFloat(apElem->Attribute("DestValue"),0); float
  fMaxOutput = cString::ToFloat(apElem->Attribute("MaxOutput"),0);

  ePhysicsControllerOutput CtrlOutput =
  GetControllerOutput(apElem->Attribute("Output")); ePhysicsControllerAxis
  CtrlOutputAxis = GetControllerAxis(apElem->Attribute("OutputAxis")); bool
  bMulMassWithOutput =
  cString::ToBool(apElem->Attribute("MulMassWithOutput"),false);

  ePhysicsControllerEnd CtrlEnd =
  GetControllerEnd(apElem->Attribute("EndType")); tString sNextCtrl =
  cString::ToString(apElem->Attribute("NextController"),"");
  ...
  //	sName.c_str(),bActive, fA,fB,fC, (int)CtrlInput, (int)CtrlInputAxis,
  (int)CtrlOutput,(int)CtrlOutputAxis);*/
  }

  //-----------------------------------------------------------------------

  eAnimationEventType
  cEntityLoader_Object::GetAnimationEventType(const char* apString) {
    if (apString == NULL)
      return eAnimationEventType_LastEnum;
  ...
  void cEntityLoader_Object::LoadUserVariables(cXmlElement* apRootElem) {
    cXmlElement* pVarRootElem =
        apRootElem->GetFirstElement("UserDefinedVariables");
