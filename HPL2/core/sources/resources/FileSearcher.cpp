...
namespace hpl {
  ...
  void cFileSearcher::AddDirectory(const tWString& asSearchPath, const tString& asMask, bool abAddSubDirectories) {
    ...
    for (auto it = lstFileNames.begin(); it != lstFileNames.end(); it++) {
      tWString& sFile     = *it;
      ...
      auto pathIt = m_mapFiles.find(sLowFile);
    ...
    if (abAddSubDirectories) {
      ...
      for (auto it = lstDirNames.begin(); it != lstDirNames.end(); it++) {
  ...
  const tWString& cFileSearcher::GetFilePath(const tString& asFileNameAndPath, int* apEqualCount) {
    ...
    auto it = m_mapFiles.find(sLowName);
    ...
    if (lCount == 1 && apEqualCount == nullptr) {
    ...
    for (size_t itcount = 0; itcount < lCount; ++itcount, ++it) {
      ...
      int lEqualCount1 = 0;
      int j            = static_cast<int>(it->second.mvPathDirs.size()) - 1;
      for (int i = static_cast<int>(vWantedDirs.size()) - 1; (i >= 0 && j >= 0); --j) {
      ...
      int lEqualCount2 = 0;
      j                = static_cast<int>(vWantedDirs.size()) - 1;
      for (int i = static_cast<int>(it->second.mvPathDirs.size()) - 1; (i >= 0 && j >= 0); --j) {
        //if equal, increase equal count and go to next wanted dir
