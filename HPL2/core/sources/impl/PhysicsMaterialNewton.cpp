...
namespace hpl {
  ...
  cPhysicsMaterialNewton::cPhysicsMaterialNewton(const tString& asName, iPhysicsWorld* apWorld, int alMatId)
    ...
    auto* pNWorld = dynamic_cast<cPhysicsWorldNewton*>(mpWorld);
  ...
  void cPhysicsMaterialNewton::UpdateMaterials() {
    ...
    while (MatIt.HasNext()) {
      auto* pMat = dynamic_cast<cPhysicsMaterialNewton*>(MatIt.Next());

      auto frictionMode   = (ePhysicsMaterialCombMode) std::max(mFrictionMode, pMat->mFrictionMode);
      auto elasticityMode = (ePhysicsMaterialCombMode) std::max(mElasticityMode, pMat->mElasticityMode);
      ...
      NewtonMaterialSetCollisionCallback(mpNewtonWorld, mlMaterialId, pMat->mlMaterialId,
                                         (void*) nullptr, OnAABBOverlapCallback, ContactsProcessCallback);
    }
  }
  ...
  int cPhysicsMaterialNewton::OnAABBOverlapCallback(const NewtonMaterial* apMaterial,
                                                    ...
                                                    int alThreadIndex) {
    auto* pContactBody1 = (cPhysicsBodyNewton*) NewtonBodyGetUserData(apBody1);
    auto* pContactBody2 = (cPhysicsBodyNewton*) NewtonBodyGetUserData(apBody2);

    if ((pContactBody1->GetCollideFlags() & pContactBody2->GetCollideFlags()) == 0) {
      return 0;
    }

    if (!pContactBody1->GetCollide()) {
      return 0;
    }
    if (!pContactBody2->GetCollide()) {
      return 0;
    }

    if (!pContactBody1->IsActive()) {
      return 0;
    }
    if (!pContactBody2->IsActive()) {
      return 0;
    }

    if (pContactBody1->IsRagDoll() && !pContactBody2->GetCollideRagDoll()) {
      return 0;
    }
    if (pContactBody2->IsRagDoll() && !pContactBody1->GetCollideRagDoll()) {
      return 0;
    }

    if (pContactBody1->IsCharacter() && !pContactBody2->GetCollideCharacter()) {
      return 0;
    }
    if (pContactBody2->IsCharacter() && !pContactBody1->GetCollideCharacter()) {
      return 0;
    }
    ...
    //Call the callbacks
    if (!pContactBody1->OnAABBCollision(pContactBody2)) {
      return 0;
    }
    if (!pContactBody2->OnAABBCollision(pContactBody1)) {
      return 0;
    }
  ...
  void cPhysicsMaterialNewton::ContactsProcessCallback(const NewtonJoint* apContactJoint, dFloat afTimestep, int alThreadIndex) {
    ...
    auto* pContactBody1 = (cPhysicsBodyNewton*) NewtonBodyGetUserData(pBody0);
    auto* pContactBody2 = (cPhysicsBodyNewton*) NewtonBodyGetUserData(pBody1);
    ...
    for (; pContact != nullptr; pContact = NewtonContactJointGetNextContact(apContactJoint, pContact)) {
      ...
      //Force
      cVector3f vForce;
      NewtonMaterialGetContactForce(pMaterial, pBody0, vForce.v); // todo: mhpl what body?
      contactData.mvForce += vForce;

      //Position and normal
      cVector3f vPos, vNormal;
      NewtonMaterialGetContactPositionAndNormal(pMaterial, pBody0, vPos.v, vNormal.v); // todo: mhpl what body?
      ...
      if (pContactBody1->GetWorld()->GetSaveContactPoints()) {
        ...
        NewtonMaterialGetContactPositionAndNormal(pMaterial, pBody0, collidePoint.mvPoint.v, collidePoint.mvNormal.v); // todo: mhpl what body?
    ...
    if (pMaterial1->GetSurfaceData() && pMaterial2->GetSurfaceData() &&
        ...
        !pContactBody1->GetBuoyancyActive() && !pContactBody2->GetBuoyancyActive()) {
