...
namespace hpl {
  ...
  void cPhysicsBodyNewtonCallback::OnTransformUpdate(iEntity3D* apEntity) {
    if (!cPhysicsBodyNewton::mbUseCallback) {
      return;
    }
    auto* pRigidBody = dynamic_cast<cPhysicsBodyNewton*>(apEntity);
    NewtonBodySetMatrix(pRigidBody->mpNewtonBody, apEntity->GetLocalMatrix().GetTranspose().v);
  }
  ...
  void cPhysicsBodyNewton::SetMaterial(iPhysicsMaterial* apMaterial) {
    ...
    auto* pNewtonMat = dynamic_cast<cPhysicsMaterialNewton*>(mpMaterial);
  ...
  void cPhysicsBodyNewton::SetMass(float afMass) {
    auto* pShapeNewton = dynamic_cast<cCollideShapeNewton*>(mpShape);
  ...
  void cPhysicsBodyNewton::OnTransformCallback(const NewtonBody* apBody, const dFloat* apMatrix, int alThreadIndex) {
    auto* pRigidBody = (cPhysicsBodyNewton*) NewtonBodyGetUserData(apBody);
