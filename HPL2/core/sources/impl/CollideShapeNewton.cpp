...
namespace hpl {
  ...
  void cCollideShapeNewton::CreateStaticSceneFromShapeVec(tCollideShapeVec& avShapes, tMatrixfVec* apMatrices) {
    ...
    for (auto& avShape : avShapes) {
      mvSubShapes.push_back(avShape);
      auto* pNewtonShape = dynamic_cast<cCollideShapeNewton*>(avShape);
      auto  matrix       = cMatrixf::Identity; // TODO: mhpl этого тут не было
      /*NewtonSceneProxy* pProxy = */
      NewtonSceneCollisionCreateProxy(mpNewtonCollision, pNewtonShape->GetNewtonCollision(), matrix.v);
