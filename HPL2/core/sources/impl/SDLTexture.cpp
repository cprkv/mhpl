  ...
  void cSDLTexture::GenerateMipMaps(GLenum aGLTarget, ePixelFormat aPixelFormat, const cVector3l avSize,
    ...
    if (mUsage == eTextureUsage_RenderTarget) {
      ...
      if (mType == eTextureType_1D) {
        ...
        gluBuild2DMipmaps(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : aGLTarget,
  ...
  bool cSDLTexture::CopyTextureDataToGL(int alTextureHandle, int alLevel, unsigned char* apData, int alDataSize,
    ...
    if (PixelFormatIsCompressed(aPixelFormat)) {
      ...
      if (mType == eTextureType_1D) {
        ...
        glCompressedTexImage2DARB(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : GLTarget,
                                  alLevel,
                                  GLCompressionFormat,
                                  avSize.x, avSize.y,
                                  0, alDataSize, apData);
    ...
    // Load uncompressed data
      ...
      if (mType == eTextureType_1D) {
        ...
        glTexImage2D(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : GLTarget,
