...
namespace hpl {
  ...
  void iFontData::GetWordWrapRows(float afLength, float afFontHeight, cVector2f avSize,
    ...
    for (pos = 0; pos < asString.size(); pos++) {
      ...
      if (asString[pos] == _W(' ') || asString[pos] == _W('\n') || IsChineseFullwidthChar(asString[pos])) {
        ...
        bool nothing = true;
        if (fTextLength > afLength && !IsChineseFullwidthChar(asString[pos])) {
          rows++;
          ...
          last_space   = pos;
          nothing      = false;
        } else if (fTextLength > afLength && IsChineseFullwidthChar(asString[pos])) {
          row.mbIncr = false;
    ...
    if (rows == 0) {
      ...
      for (auto it = rowLengthList.begin(); it != rowLengthList.end(); ++it) {
  ...
  cGlyph* iFontData::CreateGlyph(cFrameSubImage* apImage, const cVector2l& avOffset, const cVector2l& avSize,
    ...
    //Sizes
    cVector2f vSize;
    vSize.x = static_cast<float>(avSize.x) / static_cast<float>(avFontSize.x) * mvSizeRatio.x;
    vSize.y = static_cast<float>(avSize.y) / static_cast<float>(avFontSize.y) * mvSizeRatio.y;

    cVector2f vOffset;
    vOffset.x = static_cast<float>(avOffset.x) / static_cast<float>(avFontSize.x) * mvSizeRatio.x;
    vOffset.y = static_cast<float>(avOffset.y) / static_cast<float>(avFontSize.y) * mvSizeRatio.y;

    float fAdvance = static_cast<float>(alAdvance) / static_cast<float>(avFontSize.x) * mvSizeRatio.x;

    auto* pGlyph = hplNew(cGlyph, (pGuiGfx, vOffset, vSize, fAdvance));
