...
#define kContractSize (0.001f)
  ...
  cFrameSubImage::cFrameSubImage(const tString& asName, const tWString& asFullPath, cFrameTexture* apFrameTex,
    ...
    cVector2f vTexSize = cVector2f(static_cast<float>(mRect.w), static_cast<float>(mRect.h)) /
                         cVector2f(static_cast<float>(mvSourceSize.x), static_cast<float>(mvSourceSize.y));
    cVector2f vTexPos = cVector2f(static_cast<float>(mRect.x), static_cast<float>(mRect.y)) /
                        cVector2f(static_cast<float>(mvSourceSize.x), static_cast<float>(mvSourceSize.y));

    mvVtx.push_back(cVertex(cVector3f(0, 0, 0),
                            cVector3f(vTexPos.x + kContractSize, vTexPos.y + kContractSize, 0), cColor(1)));

    mvVtx.push_back(cVertex(cVector3f(static_cast<float>(mRect.w), 0, 0),
                            cVector3f(vTexPos.x + vTexSize.x - kContractSize, vTexPos.y + kContractSize, 0),
                            cColor(1)));

    mvVtx.push_back(cVertex(cVector3f(static_cast<float>(mRect.w), static_cast<float>(mRect.h), 0),
                            cVector3f(vTexPos.x + vTexSize.x - kContractSize, vTexPos.y + vTexSize.y - kContractSize, 0),
                            cColor(1)));

    mvVtx.push_back(cVertex(cVector3f(0, static_cast<float>(mRect.h), 0),
  ...
  cFrameSubImage::~cFrameSubImage() {
    ...
    if (mpFrameBitmapImage) {
      mpFrameBitmapImage->mpSubImage = nullptr; //Since we are deleting, it is no longer valid.
    }

    mvVtx.clear();
    //mpFrameTexture->DecPicCount();
    mpFrameTexture = nullptr;
    mlHandle       = -1;
  }
