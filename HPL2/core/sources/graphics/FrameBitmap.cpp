...
namespace hpl {
  ...
  static constexpr const bool DEBUG_BTREE{ false };
  ...
  cFrameSubImage* cFrameBitmap::AddBitmap(cBitmap* apSrc, const tWString& asFullPath, cFrameSubImage* apSubImageCreated, bool* apFoundNode) {
    cFBitmapImage*  pBitmapImage = nullptr;
    cFrameSubImage* pImage       = nullptr;
    //source size
    ...
    if (bFoundNode && apSubImageCreated == nullptr) {
  ...
  using tFrameBitmapSortSet   = std::multiset<cBitmapSubImageComb*, cFrameBitmapCompare>;
  using tFrameBitmapSortSetIt = tFrameBitmapSortSet::iterator;

  using tFrameBitmapCombList   = std::list<cBitmapSubImageComb*>;
  using tFrameBitmapCombListIt = tFrameBitmapCombList::iterator;
  ...
  void cFrameBitmap::Reorganize() {
    ...
    for (auto imageIt = mlstImages.begin(); imageIt != mlstImages.end(); ++imageIt) {
      cFBitmapImage* pImage = *imageIt;
      if (pImage->mpSubImage == nullptr) {
        continue;
      }
      ...
      //Add bitmap to list
      auto* pBmpComb = hplNew(cBitmapSubImageComb, (pBitmap, pImage->mpSubImage));
    ...
    for (auto bmpIt = setSortedBitmaps.begin(); bmpIt != setSortedBitmaps.end(); ++bmpIt) {
    ...
    if (!bAllNodesFit) {
      ...
      for (auto it = lstBitmaps.begin(); it != lstBitmaps.end(); ++it) {
        ...
        if (!bFit) {
          ...
          pComb->mpSubImage->mpFrameBitmapImage = nullptr;
        }
      }
    }
