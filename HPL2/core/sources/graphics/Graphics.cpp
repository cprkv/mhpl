...
namespace hpl {
  ...
  cGraphics::cGraphics(iLowLevelGraphics* apLowLevelGraphics, iLowLevelResources* apLowLevelResources)
    ...
    mpMeshCreator       = nullptr;
    mpTextureCreator    = nullptr;
    mpDecalCreator      = nullptr;
  }
  ...
  cGraphics::~cGraphics() {
    ...
    for (auto& [_, pType] : m_mapMaterialTypes) {
      pType->DestroyData();
    }
    ...
    for (auto& mvRenderer : mvRenderers) {
      if (mvRenderer) {
        mvRenderer->DestroyData();
        hplDelete(mvRenderer)
      }
    }
    mvRenderers.clear();
  ...
  bool cGraphics::Init(int alWidth, int alHeight, int alDisplay, int alBpp, int abFullscreen,
    ...
    if (alHplSetupFlags & eHplSetup_Screen) {
      ...
      mvRenderers.resize(eRenderer_LastEnum, nullptr);
  ...
  void cGraphics::Update(float afTimeStep) {
    for (auto* pRenderer : mvRenderers) {
  ...
  iRenderer* cGraphics::GetRenderer(eRenderer aType) {
    if (aType >= static_cast<int>(mvRenderers.size())) {
      return nullptr;
    }
  ...
  iFrameBuffer* cGraphics::GetTempFrameBuffer(const cVector2l& avSize, ePixelFormat aPixelFormat, int alIndex) {
    ...
    for (auto& tempBuffer : mvTempFrameBuffers) {
      if (tempBuffer.mvSize == avSize &&
    ...
    //Create texture
    tString   sNameSuffix = cString::ToString(avSize.x) + "x" + cString::ToString(avSize.y) + ":" + cString::ToString(static_cast<int>(aPixelFormat));
    iTexture* pTexture    = CreateTexture("TempBufferTexture" + sNameSuffix, eTextureType_Rect, eTextureUsage_RenderTarget);
    pTexture->CreateFromRawData(cVector3l(avSize.x, avSize.y, 0), aPixelFormat, nullptr);
    ...
    if (!pFrameBuffer->CompileAndValidate()) {
  ...
  iDepthStencilBuffer* cGraphics::CreateDepthStencilBuffer(const cVector2l& avSize, int alDepthBits, int alStencilBits, bool abLookForMatchingFirst) {
    iDepthStencilBuffer* pBuffer = nullptr;
  ...
  iDepthStencilBuffer* cGraphics::FindDepthStencilBuffer(const cVector2l& avSize, int alMinDepthBits, int alMinStencilBits) {
    for (auto* pBuffer : mlstDepthStencilBuffers) {
  ...
  cPostEffectComposite* cGraphics::CreatePostEffectComposite() {
    auto* pComposite = hplNew(cPostEffectComposite, (this));
  ...
  iPostEffect* cGraphics::CreatePostEffect(iPostEffectParams* apParams) {
    auto* pType = static_cast<iPostEffectType*>(STLFindByName(mvPostEffectTypes, apParams->GetName()));
    if (pType == nullptr) {
  ...
  iMaterialType* cGraphics::GetMaterialType(const tString& asName) {
    ...
    auto it = m_mapMaterialTypes.find(sLowName);
  ...
  tStringVec cGraphics::GetMaterialTypeNames() {
    tStringVec vNames;
    auto       it = m_mapMaterialTypes.begin();
