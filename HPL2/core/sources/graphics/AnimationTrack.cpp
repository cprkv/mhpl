...
namespace hpl {
  ...
  cKeyFrame cAnimationTrack::GetInterpolatedKeyFrame(float afTime, bool bLoop) {
    ...
    cKeyFrame* pKeyFrameA = nullptr;
    cKeyFrame* pKeyFrameB = nullptr;
    ...
    if (fT == 0.0f) {
      ...
      //      if (rotA.v.x == -1 * rotB.v.x && rotA.v.y == -1 * rotB.v.y && rotA.v.z == -1 * rotB.v.z && rotA.w == -1 * rotB.w) {
  ...
  float cAnimationTrack::GetKeyFramesAtTime(float afTime, cKeyFrame** apKeyFrameA, cKeyFrame** apKeyFrameB, bool bLoop) {
    ...
    const int lSize = static_cast<int>(mvKeyFrames.size());
