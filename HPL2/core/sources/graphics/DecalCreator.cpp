...
namespace hpl {
  ...
  bool cDecalCreator::Compile() {
    if (mbUpdated) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        for (float& j : vSign) {
          mvClipPlanes.push_back(cPlanef(vAxes[i] * j, mvDecalPosition - vAdd * j));
      ...
      ///////////////////////////////////////////
      // Clip near triangles
      for (auto pAffectedMesh : mvMeshes) {
      ...
      auto      fStartU        = static_cast<float>(mlCurrentSubDiv % mvSubDiv.x);
      auto      fStartV        = static_cast<float>(mlCurrentSubDiv / mvSubDiv.x);
  ...
  cMesh* cDecalCreator::CreateDecalMesh() {
    ...
    auto*     pMesh    = hplNew(cMesh, ("", _W(""), mpResources->GetMaterialManager(), mpResources->GetAnimationManager()));
  ...
  void cDecalCreator::DrawDebug(cRendererCallbackFunctions* apFunctions, bool abDrawAxes, bool abDrawWireframe) {
    ...
    if (mpDecalVB) {
      if (mpDecalMaterial) {
        ...
        apFunctions->SetProgram(nullptr);
        ...
        apFunctions->SetTextureRange(nullptr, 0);
  ...
  void cDecalCreator::ClipMesh(cSubMeshEntity* apSubMesh, iVertexBuffer* apDecalVB) {
    ...
    for (auto& mvClipPlane : mvClipPlanes) {
      vTransformedPlanes.push_back(cMath::TransformPlane(mtxInvSubMeshWorldMatrix, mvClipPlane));
    ...
    for (int j = 0; j < pSubMeshVB->GetIndexNum(); j += 3) {
      ...
      if ((lCount != 0) && !AddPolygon(lCount, vNewVertices, vNewNormals, apDecalVB, mtxSubMeshWorldMatrix, mtxSubMeshWorldNormalRot)) {
        break;
      }
    }
  }
