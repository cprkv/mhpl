...
namespace hpl {
  ...
  cBone* cBone::CreateChildBone(const tString& asName, const tString& asId) {
    auto* pBone     = hplNew(cBone, (asName, asId, mpSkeleton));
    pBone->mpParent = this;
  ...
  void cBone::Detach() {
    ...
    for (auto it = mpParent->mlstChildren.begin(); it != mpParent->mlstChildren.end(); it++) {
      if (*it == this) {
