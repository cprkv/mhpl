...
#define kVar_afSkipEdgeLimit 22
  ...
  bool cRendererDeferred::LoadData() {
    ...
    for (int type = 0; type < 2; ++type) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_Full]->CompileAndValidate()) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_Color]->CompileAndValidate()) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_Depth]->CompileAndValidate()) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_ColorAndDepth]->CompileAndValidate()) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_Normals]->CompileAndValidate()) {
      ...
      if (!mpGBuffer[type][eGBufferComponents_LinearDepth]->CompileAndValidate()) {
    ...
    if (mpLowLevelGraphics->GetCaps(eGraphicCaps_ShaderModel_4)) {
      if (mShadowMapQuality == eShadowMapQuality_High) {
        ...
        mlShadowJitterSamples = 32; // todo 64 here instead? I mean, ATI has to deal with medium has max? or different max for ATI?
    ...
    //Create Fog program
    {
      ...
      mpFogProgramManager->SetupGenerateProgramData(0, "Fog",
                                                    "deferred_fog_vtx.glsl", "deferred_fog_frag.glsl",
                                                    gvFogAreaFeatureVec, kFogAreaFeatureNum, vars);
    ...
    //Create Light programs
    {
      ...
      //Lights
      {
        ...
        //Shadow variables
        defaultVars.Add("ShadowJitterLookupMul", 1.0f / static_cast<float>(mlShadowJitterSize));
    ...
    if (mbSSAOLoaded) {
      ...
      for (auto& i : mpSSAOBlurProgram) {
        if (i) {
          i->GetVariableAsId("afFarPlane", kVar_afFarPlane);
        }
      }
